...
---------
    ...
    |        |
    |-270R---|  2      PE4      (dis)charge out or Z
             |
...
static void setup_ports() {
    ...
    */
    ...
    DDRE = B00010000;  // PE4 output, others input
    PORTE = B11100100    | // 1=unused, WPU
            (B00 << PE0) | // UART, leave alone
            (0 << PE3)   | // AIN1, no WPU
            (1 << PE4);    // drive high, initially discharge
...
static void dump() {
    ...
    Serial.print(" TIFR1=B"); Serial.println(TIFR1, BIN);
}

static void charge() {
    start_capture();
    
    PORTE &= ~(1 << PE4); // drive low
}
...
static void discharge() {
    PORTE |= 1 << PE4; // drive high
}

static volatile bool refresh_now = false;

static void wait_refresh() {
    // while (!refresh_now);      // wait for compare
    // refresh_now = false;
    TIFR3 |= 1 << OCF3A; // "clear" the interrupt flag
    while (!(TIFR3 & (1 << OCF3A)));
}
