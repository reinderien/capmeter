...
Choose maximum R based on the impedance of the pins. Anywhere above 1M and
the discharge accuracy is affected.
...
static void setup_comptor() {
    ...
    ACIS has no effect here because we do not use the AC interrupt itself, but rather
    the timer capture interrupt. As such, only TCCR1B.ICES determines the edge.
     */
    ACSR = (0 << ACD)  | // comparator enabled
           ...
           (B10 << ACIS0); // AC interrupt on falling edge (no effect)
...
static void setup_capture() {
    ...
    */
    ...
    // Per 17.6.1 p141, this must be done after a change to ACIC
    TIFR1 = 0xFF; // "Clear" all timer 1 interrupt flags
}

static void start_capture() {
    ICR1 = 0;  // Clear capture value
...
static void print_cap(uint16_t icr) {
    const float taus = 1.514128, // ln(5/1.1)
                t = ((float)icr)/F_CPU*ranges[r_index].prescale,
                R = ranges[r_index].R;
    float C = t/taus/R;
...
static void dump() {
    ...
    Serial.print(" icr=");    Serial.print(icr_captured, DEC);
    Serial.print(' ');
}
...
static void discharge() {
    ...
    if (state == state_captured) {
        ...
        Serial.print('=');
        print_cap(icr_captured);
        
        // Calculate a better range
        for (uint16_t icr = icr_captured; icr < ranges[r_index].min; icr *= ranges[r_index].grow)
            r_index++;
    }
    else if (state == state_overflowed) {
        PORTB &= B01111111;
        Serial.print('>');
        print_cap(0xFFFF);
...
void loop() {
    for (;;) { // do not allow serialEvent
        while (state != state_discharged);
        charge();
        while (state == state_charging);
        while (state != state_captured && state != state_overflowed);
        discharge();
        while (state == state_discharging);
    }
}    
